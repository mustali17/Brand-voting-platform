...
export default function CategoryForm({
  ...
  const [toggleCategoryVisibility] = useToggleCategoryVisibilityMutation();
    ...
    try {
      if (isEditMode) {
        ...
        if (updateResponse.success) {
          ...
          if (formData.hidden !== initialData?.hidden) {
            const visibilityResponse = await toggleCategoryVisibility(
              formData._id || ""
            ).unwrap();
            if (visibilityResponse.success) {
              toast.success(
                `Category is now ${visibilityResponse.hidden ? "hidden" : "visible"}`
              );
            }
          }
